...
namespace
{
    ...
    std::array<RegistrySlot, 2> g_registry{};
    ...
    auto family_for_mesh(UObject* mesh) -> std::int32_t
    {
        ...
        if (contains_fragment(name, STR("Man001A_HELM")))
        {
            return 0;
        }
        if (contains_fragment(name, STR("Man002A_HELM")))
        {
            return 1;
        }
        return -1;
...
namespace ZeroCompanyMandoWardrobe::HelmetRenderFit
{
    auto initialize() -> bool
    {
        ...
        if (!validate_runtime(g_runtime, reason))
        {
            RC::Output::send<RC::LogLevel::Error>(
                STR("[ZeroCompanyMandoWardrobe] helmet_render_fit_REFUSED reason={} hooks_active=false\n"),
        ...
        if (!g_broadcast_refreshed_hook->hook() || g_broadcast_refreshed_trampoline == 0)
        {
            unhook_all();
            RC::Output::send<RC::LogLevel::Error>(
                STR("[ZeroCompanyMandoWardrobe] helmet_render_fit_REFUSED reason=refresh-hook-install-failed hooks_active=false\n"));
            return false;
        }
            ...
            g_update_previous_ref_to_local_trampoline == 0)
        {
            unhook_all();
            RC::Output::send<RC::LogLevel::Error>(
                STR("[ZeroCompanyMandoWardrobe] helmet_render_fit_REFUSED reason=previous-matrix-hook-install-failed hooks_active=false\n"));
            return false;
        }
        ...
        if (!g_update_ref_to_local_hook->hook() || g_update_ref_to_local_trampoline == 0)
        {
            unhook_all();
            RC::Output::send<RC::LogLevel::Error>(
                STR("[ZeroCompanyMandoWardrobe] helmet_render_fit_REFUSED reason=current-matrix-hook-install-failed hooks_active=false\n"));
            return false;
        }
    ...
    auto update() -> void
    {
        ...
        {
            RC::Output::send<RC::LogLevel::Verbose>(
                STR("[ZeroCompanyMandoWardrobe] helmet_render_fit_refresh count={} scan_pending=true\n"),
                refreshes);
        }
        ...
        if (++g_summary_frames >= kSummaryIntervalFrames)
        {
            ...
            if (snapshot_nonzero(snapshot))
            {
                ...
                RC::Output::send<RC::LogLevel::Verbose>(
                    STR("[ZeroCompanyMandoWardrobe] helmet_render_fit_summary current_target={} previous_target={} current_validated={} previous_validated={} current_applied={} previous_applied={} matrices={} registry_refused={} output_refused={} leader_refused={} transform_refused={} deformer_refused={} mutation_enabled=true\n"),
    ...
    auto shutdown() -> void
    {
        ...
        const CounterSnapshot snapshot = take_counters();
        RC::Output::send<RC::LogLevel::Verbose>(
            STR("[ZeroCompanyMandoWardrobe] helmet_render_fit_unloaded hooks_active=false current_applied={} previous_applied={} matrices={} refusals={} scene_transform_writes=false\n"),
            snapshot.current_components_applied,
            snapshot.previous_components_applied,
            snapshot.matrices_applied,
